...
app.layout = html.Div([
    html.Div([
        dcc.Slider(id="year", min=1952, max=2007, step=5,
...
def plotly_fig(year):
    ...
    size_max=30, color="continent", log_x=True, height=400, width=600,
...
def altair_fig(year):
    df = gapminder.query("year == %d" % (year or 1952))
    return alt.Chart(df,height=250, width=400).mark_circle().encode(
...
def bokeh_fig(year):
    df = gapminder.query("year == %d" % (year or 1952))
    return json_item(hv.render(
        hv.Points(df, kdims=["gdpPercap", "lifeExp"]).opts(
            color="continent", size=hv.dim('pop')**(0.5)/800, logx=True,
            height=330, width=530,
            cmap='Category10',legend_position='bottom_right',
            title="HoloViews / Bokeh", tools=["hover"])
    ))
...
def seaborn_fig(year):
    ...
    sns.scatterplot(data=df, ax=ax, x='gdpPercap', y='lifeExp',
                            hue="continent", size="pop", sizes=(0,800))
    ax.set_xscale('log')
